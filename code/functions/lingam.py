...
def lingam_ecn_boot(epochs, channels, maxlag=4, current_subject=None, verbose=True):
    ...
    (mean strengths and probabilities are returned).
    ...
    all_strengths_df = {} # inst_&_lags to strengths
    all_probs_df = {} # inst_&_lags to probabilities
    ...
    for e, epoch in enumerate(epochs):
        ...
        # 3. reliability: bootstrap (***@jit, my optimization: from https://github.com/cdt15/lingam/issues/130 ***)
        ...
        probs = boot_res.get_probabilities(min_causal_effect=0.2) # 0.2 as in VARLiNGAM example on git
...
#%% reliability experiments: jackknife and Wald test
# ---------- jackknife ---------- 
# -> leave-one-epoch-out
...
# varlingam
def lingam_ecn_wald(epochs, channels, maxlag=4, current_subject=None, verbose=True):
    """
    Compute VAR-LiNGAM (time-series) ECN for one subject by aggregating across epochs.
    ...
    mean_strength : dict
        maps each lag to its pd.DataFrame of causal strengths (n_channels, n_channels)
    mean_pvals : dict
        maps each lag to its pd.DataFrame of causal pvals from Wald test (n_channels, n_channels)
    """
    ...
    for e, epoch in enumerate(epochs):
        ...
        # pvals_independence = model.get_error_independence_p_values() # only for B0 (instantaneous) :(
